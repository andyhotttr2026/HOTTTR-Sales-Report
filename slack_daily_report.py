...
table_lines = []
col_w = 22
for name, s in sorted_creators:
    subs_str = f"{s['new']+s['ren']} ({s['new']} new/{s['ren']} ren)" if not s["free"] else "free page"
    tag      = " [free]" if s["free"] else ""
    row      = f"{(name+tag):<{col_w}}  ${s['net']/100:>8.2f}    {subs_str}"
    table_lines.append(row)

table_lines.append("─" * 54)
table_lines.append(f"{'TOTAL':<{col_w}}  ${grand_net/100:>8.2f}    {total_subs} ({grand_new} new/{grand_ren} ren)")
